...
//    private boolean taskFailed[];
    ...
    public MultiTask(String fileName, final URL url, final FileInfo entity, IDownloadStateListener downloadStateListeners) {
...
//        Log.d("WEN", "MultiTask notify onPauseOption" );
    ...
    public void onProcess(int threadId, long contentLength, long completeLength, int percent) {
        ...
        lengthArray[threadId - 1] = contentLength;
        long tmp = 0L;
        for (int i = 1; i <= MAX_THREAD_SIZE; i++) {
            tmp += lengthArray[i - 1];
        }
