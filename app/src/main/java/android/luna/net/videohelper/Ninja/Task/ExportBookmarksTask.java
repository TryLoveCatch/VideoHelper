...
public class ExportBookmarksTask extends AsyncTask<Void, Void, Boolean> {
    ...
    protected Boolean doInBackground(Void... params) {
        path = android.luna.net.videohelper.Ninja.Unit.BrowserUnit.exportBookmarks(context);
    ...
    protected void onPostExecute(Boolean result) {
        ...
        if (result) {
            ToastUtils.show(context, context.getString(R.string.toast_export_bookmarks_successful) + path);
        } else {
            ToastUtils.show(context, R.string.toast_export_bookmarks_failed);
        }
    }
}
