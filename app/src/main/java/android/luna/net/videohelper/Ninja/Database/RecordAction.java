...
public class RecordAction {
    ...
    private RecordAction(Context context) {
        this.helper = new RecordHelper(context);
    }

    public static RecordAction getInstance(Context context) {
        if (instance == null) {
    ...
    public void open(boolean rw) {
        synchronized (RecordAction.this) {
            if (rw) {
    ...
    public boolean addPlayRecord(Record record) {
        if (record == null
                || record.getTitle() == null
                || record.getTitle().trim().isEmpty()
                || record.getURL() == null
        ...
        values.put(RecordUnit.COLUMN_TITLE, record.getTitle().trim());
        values.put(RecordUnit.COLUMN_URL, record.getURL().trim());
        values.put(RecordUnit.COLUMN_PLAY_TIME, record.getPlayTime());
        values.put(RecordUnit.COLUMN_TIME, record.getTime());
        database.insert(RecordUnit.TABLE_PLAY_RECORD, null, values);
    ...
    public boolean addGridItem(android.luna.net.videohelper.Ninja.View.GridItem item) {
        if (item == null
                || item.getTitle() == null
    ...
    public boolean updatePlayRecord(Record record) {
        if (record == null
                || record.getTitle() == null
                || record.getTitle().trim().isEmpty()
                || record.getURL() == null
        ...
        values.put(RecordUnit.COLUMN_PLAY_TIME, record.getPlayTime());
        values.put(RecordUnit.COLUMN_TIME, record.getTime());
        database.update(RecordUnit.TABLE_PLAY_RECORD, values, RecordUnit.COLUMN_URL + "=?", new String[]{String.valueOf(record.getURL())});

        return true;
    }

    public boolean updateGridItem(android.luna.net.videohelper.Ninja.View.GridItem item) {
        if (item == null
                || item.getTitle() == null
                || item.getTitle().trim().isEmpty()
                || item.getURL() == null
    ...
    public boolean checkPlayRecord(String url) {
        if (url == null || url.trim().isEmpty()) {
            return false;
        }

        Cursor cursor = database.query(
                RecordUnit.TABLE_PLAY_RECORD,
                new String[]{RecordUnit.COLUMN_URL},
                RecordUnit.COLUMN_URL + "=?",
                new String[]{url.trim()},
                null,
    ...
    public boolean checkGridItem(android.luna.net.videohelper.Ninja.View.GridItem item) {
    ...
    public boolean deletePlayRecord(Record record) {
        if (record == null || record.getURL() == null || record.getURL().trim().isEmpty()) {
            return false;
        }

        database.execSQL("DELETE FROM " + RecordUnit.TABLE_PLAY_RECORD + " WHERE " + RecordUnit.COLUMN_URL + " = " + "\"" + record.getURL().trim() + "\"");
        return true;
    }
    ...
    public boolean deleteGridItem(android.luna.net.videohelper.Ninja.View.GridItem item) {
        if (item == null || item.getURL() == null || item.getURL().trim().isEmpty()) {
            return false;
        }
    ...
    public void clearPlayRecords() {
        database.execSQL("DELETE FROM " + RecordUnit.TABLE_PLAY_RECORD);
    }
    ...
    private Record getPlayRecord(Cursor cursor) {
        Record record = new Record();
        record.setTitle(cursor.getString(0));
        record.setURL(cursor.getString(1));
        record.setPlayTime(cursor.getLong(2));
        record.setTime(cursor.getLong(3));
        return record;
    }

    private android.luna.net.videohelper.Ninja.View.GridItem getGridItem(Cursor cursor) {
        android.luna.net.videohelper.Ninja.View.GridItem item = new android.luna.net.videohelper.Ninja.View.GridItem();
    ...
    public List<Record> listPlayRecords() {
        List<Record> list = new ArrayList<>();
    ...
    public List<android.luna.net.videohelper.Ninja.View.GridItem> listGrid() {
        List<android.luna.net.videohelper.Ninja.View.GridItem> list = new LinkedList<>();
