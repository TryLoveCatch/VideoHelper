...
public final class CaptureActivity extends Activity implements SurfaceHolder.Callback, View.OnClickListener {
    ...
    public void onCreate(Bundle icicle) {
        ...
        infoText = (TextView) findViewById(R.id.capture_mask_top);
        inactivityTimer = new InactivityTimer(this);
        beepManager = new BeepManager(this);
        TextView title = (TextView) findViewById(R.id.title);
        title.setText("扫一扫");

        String str = "<font color='#9e9e9e'>请在电脑浏览器打开</font><br><font color='#248ae0'>http://jiasugou.tv/</font><br><font color='#9e9e9e'>扫码，即可在电脑上极速播放</font><br>"
                + "<br><font color='#248ae0'>建议使用Chrome,Safari浏览器以获得最佳体验</font>";
        infoText.setText(Html.fromHtml(str));
    ...
    public void handleDecode(Result rawResult, Bundle bundle) {
        ...
        if (!StringUtils.isBlank(link)) {
            ...
            map.put("link", linkencry);
        }
        map.put("source", rawResult.getText());
        map.put("site", site);
        if (!StringUtils.isBlank(webUrl)) {
            map.put("url", Base64.encodeToString(webUrl.getBytes(), Base64.DEFAULT));
        }
        if (!StringUtils.isBlank(webTitle)) {
            map.put("title", webTitle);
        }
        webUrl = StringUtils.nullStrToEmpty(webUrl);
        vid = StringUtils.nullStrToEmpty(vid);
        handler.setPostMap(map);
        String targetUrl = getIntent().getStringExtra("targetUrl");
        if (!StringUtils.isBlank(targetUrl)) {
            Message message = new Message();
            message.what = GlobalConstant.VIDEO_URL_RECEIVE;
            message.obj = targetUrl;
            handler.sendMessage(message);
        } else {
            UrlParseHelper helper = new UrlParseHelper(this, webUrl, site, vid, handler, true);
