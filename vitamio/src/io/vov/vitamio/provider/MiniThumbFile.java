...
public class MiniThumbFile {
  ...
  private RandomAccessFile miniThumbDataFile() {
    if (mMiniThumbFile == null) {
      ...
      if (!directory.isDirectory()) {
        if (!directory.mkdirs())
          VitamioLog.e("Unable to create .thumbnails directory %s", directory.toString());
      }
  ...
  protected synchronized long getMagic(long id) {
    ...
    if (r != null) {
      ...
      try {
        ...
        VitamioLog.e("Got exception checking file magic: ", ex);
      } catch (RuntimeException ex) {
        VitamioLog.e("Got exception when reading magic, id = %d, disk full or mount read-only? %s", id, ex.getClass().toString());
      } finally {
        try {
          if (lock != null)
            lock.release();
  ...
  protected synchronized void saveMiniThumbToFile(byte[] data, long id, long magic) throws IOException {
    ...
    try {
      ...
      VitamioLog.e("couldn't save mini thumbnail data for %d; %s", id, ex.getMessage());
      throw ex;
    } catch (RuntimeException ex) {
      VitamioLog.e("couldn't save mini thumbnail data for %d, disk full or mount read-only? %s", id, ex.getClass().toString());
    } finally {
      try {
        if (lock != null)
          lock.release();
  ...
  protected synchronized byte[] getMiniThumbFromFile(long id, byte[] data) {
    ...
    try {
      ...
      VitamioLog.e("got exception when reading thumbnail id = %d, exception: %s", id, ex.getMessage());
    } catch (RuntimeException ex) {
      VitamioLog.e("Got exception when reading thumbnail, id = %d, disk full or mount read-only? %s", id, ex.getClass().toString());
    } finally {
      try {
        if (lock != null)
          lock.release();
